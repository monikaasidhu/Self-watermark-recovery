...
checkpoint_unet = torch.load('unet_model7.pth')
checkpoint_latent_decoder = torch.load('latent_decoder_model7.pth')
...
optimizer_unet = optim.Adam(unet.parameters(), lr=0.00001)
optimizer_unet.load_state_dict(checkpoint_unet['optimizer_state_dict'])

optimizer_latent_decoder = optim.Adam(latent_decoder.parameters(), lr=0.00001)
...
dataset_path = '/home/himanshu/dataset/data/marksheet3'
image_dataset = ImageDataset(dataset_path, transform=transform)
data_loader = DataLoader(image_dataset, batch_size=1, shuffle=True)  # Use batch_size=1 for simplicity

# Loss functions and optimizers
criterion_mse = nn.MSELoss()
lpips_loss_weight = 10.0  # Weight for LPIPS loss
...
for epoch in range(num_epochs):
    ...
    for images in data_loader:
        ...
        total_loss_unet = mse_loss_unet + lpips_loss_weight * lpips_loss_unet
        ...
        total_loss_latent_decoder = criterion_mse(fout, images) + 5* lpips_loss_fn(fout, images) +  criterion_mse(encoder_output, latent_decoder_output_resize)
